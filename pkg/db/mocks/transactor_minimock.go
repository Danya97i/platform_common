...
//go:generate minimock -i github.com/Danya97i/platform_common/pkg/db.Transactor -o transactor_minimock.go -n TransactorMock -p mocks
	...
	if mmBeginTx.BeginTxMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmBeginTx.t.Errorf("TransactorMock.BeginTx got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.txOptions != nil && !minimock.Equal(*mm_want_ptrs.txOptions, mm_got.txOptions) {
				mmBeginTx.t.Errorf("TransactorMock.BeginTx got unexpected parameter txOptions, want: %#v, got: %#v%s\n", *mm_want_ptrs.txOptions, mm_got.txOptions, minimock.Diff(*mm_want_ptrs.txOptions, mm_got.txOptions))
			}
...
func (m *TransactorMock) MinimockBeginTxInspect() {
	...
	if !m.BeginTxMock.invocationsDone() && afterBeginTxCounter > 0 {
		m.t.Errorf("Expected %d calls to TransactorMock.BeginTx but found %d calls",
