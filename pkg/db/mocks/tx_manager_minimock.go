...
//go:generate minimock -i github.com/Danya97i/platform_common/pkg/db.TxManager -o tx_manager_minimock.go -n TxManagerMock -p mocks
...
import (
	...
	mm_db "github.com/Danya97i/platform_common/pkg/db"
...
// ReadCommited implements db.TxManager
	...
	if mmReadCommited.ReadCommitedMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmReadCommited.t.Errorf("TxManagerMock.ReadCommited got unexpected parameter ctx, want: %#v, got: %#v%s\n", *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.f != nil && !minimock.Equal(*mm_want_ptrs.f, mm_got.f) {
				mmReadCommited.t.Errorf("TxManagerMock.ReadCommited got unexpected parameter f, want: %#v, got: %#v%s\n", *mm_want_ptrs.f, mm_got.f, minimock.Diff(*mm_want_ptrs.f, mm_got.f))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmReadCommited.t.Errorf("TxManagerMock.ReadCommited got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *TxManagerMock) MinimockReadCommitedInspect() {
	for _, e := range m.ReadCommitedMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to TxManagerMock.ReadCommited with params: %#v", *e.params)
		}
	}
	...
	if m.ReadCommitedMock.defaultExpectation != nil && afterReadCommitedCounter < 1 {
		if m.ReadCommitedMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to TxManagerMock.ReadCommited")
		} else {
			m.t.Errorf("Expected call to TxManagerMock.ReadCommited with params: %#v", *m.ReadCommitedMock.defaultExpectation.params)
		}
	}
	...
	if !m.ReadCommitedMock.invocationsDone() && afterReadCommitedCounter > 0 {
		m.t.Errorf("Expected %d calls to TxManagerMock.ReadCommited but found %d calls",
			mm_atomic.LoadUint64(&m.ReadCommitedMock.expectedInvocations), afterReadCommitedCounter)
	}
}
